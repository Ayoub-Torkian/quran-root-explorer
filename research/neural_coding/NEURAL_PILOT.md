...
route to a trustworthy dissociation. A cheap interim hardening is extending the acoustic replication
(§3.2) to ~20 subjects for formal significance.

## Data, code & reproducibility

Narratives ds002345 (OpenNeuro). Scripts in `scripts/`: `encoding_sim.py` (validation),
`encoding_acoustic.py` + `encoding_multi.py` (step 1), `encoding_semantic.py` (step 2),
`encoding_dissociation.py` (step 3), `fetch_narratives.py` (data). Per-run logs in the `*_result.json`
files; figures N1–N3 in `figures/`. Full narrative: `FIRST_STEP.md`; scope/rationale:
`../two_books_genome/NEURAL_CODING_SCOPING.md`; programme overview: `../two_books_genome/PROGRAM_SUMMARY.md`.
