...
# ---- force LibreOffice to populate the TOC field on open (fixes empty TOC, incl. RTL) ----
try:
    d2=Document(out)
    st=d2.settings.element
    for ex in st.findall(qn('w:updateFields')):
        st.remove(ex)
    uf=OxmlElement('w:updateFields')
    uf.set(qn('w:val'),'true')
    st.insert(0,uf)
    d2.save(out)
    print("updateFields=true injected")
except Exception as e:
    print("WARN updateFields:", e)
