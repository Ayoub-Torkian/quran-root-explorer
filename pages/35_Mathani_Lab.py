...
recurring-trigram density + exact-refrain detection, placed against world oral
literature (Kalevala, Iliad) and prose. Honest finding: repetition is real but
localized and typologically oral-formulaic (below the Kalevala). No inimitability claim.
Computed live from the loaded corpus (rasm column)."""
...
k3.metric("Repetition in top-10 sūras", f"{top10*100:.0f}%")
k4.metric("Qur'ān vs Kalevala (rep.)", "0.029 vs 0.049")

layer(1, "Where the repetition concentrates")
st.caption("Recurring-trigram density per sūra. Repetition is localized — a few refrain-sūras carry "
           "most of it; the rest is no more repetitive than ordinary prose.")
top = M.head(14)
...
st.table(pd.DataFrame([{"×": c, "refrain (rasm)": a} for c, a in sel.refrains]))
...
st.markdown(
    ...
    "- **Powered vs genre-matched saj'** (Nahj al-Balāgha): the Qur'ān is measurably more repetitive "
    "(hapax d=−2.0, tight CI), even against rhymed-prose sermons.")
