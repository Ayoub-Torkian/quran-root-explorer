"""Close-up · Concept connectome + rearrangement — al-Kawthar's words + the inner self.
ONE grounded connectome (edges = distinctive PPMI association with >=3 shared verses; width = strength; hover =
the verse-count grounding). Three LAYOUTS of that same connectome (meaning / form / rarity placement) — the
relationships stay constant, only node placement changes. Edges are MEASURED on the rasm; PPMI is frequency-
controlled so the network does NOT 'relate everything'. Nodes draggable (Plotly editable). HUMAN-CONSTRUCT tagged."""
import os, json
...
st.set_page_config(page_title="Close-up · Concept connectome", page_icon="🧭", layout="wide")
if S:
...
C.hero("Concept connectome — al-Kawthar's words + the inner self",
       ...
       "The relationships are constant and grounded; only where the nodes sit changes. Computation localizes "
       "known meaning for reading — it is not a claim about the text's order.",
...
st.caption("The connectome (edges) stays the same in every view — only the node **placement** changes:")
...
fig = go.Figure()
# ONE grounded connectome, drawn in this layout. Width tiers by PPMI; hover (midpoints) carries the grounding.
_tiers = [(2.5, 3.4, 0.6, "#0F6E56"), (1.3, 2.1, 0.42, "#1D9E75"), (0.0, 1.0, 0.28, "#7FB7A2")]
...
fig.add_trace(go.Scatter(x=_mx, y=_my, mode="markers", marker=dict(size=12, color="rgba(0,0,0,0)"),
                         hovertext=_mh, hoverinfo="text", showlegend=False))
# node size-dots (df) + draggable label pills
_dfmax = max(n["df"] for n in _N) or 1
fig.add_trace(go.Scatter(x=[_N[k]["pos"][_key][0] for k in range(len(_N))],
                         y=[_N[k]["pos"][_key][1] for k in range(len(_N))], mode="markers",
                         marker=dict(size=[10 + 22 * (n["df"] / _dfmax) ** 0.5 for n in _N],
                                     color=[n["color"] for n in _N], opacity=0.20, line=dict(width=0)),
                         hovertext=["%s · %d verses" % (n["label"], n["df"]) for n in _N], hoverinfo="text", showlegend=False))
_anns = []
for nd in _N:
    x, y = nd["pos"][_key]
    bw = 1 + (2 if nd["df"] >= 100 else 1 if nd["df"] >= 25 else 0)
    _anns.append(dict(x=x, y=y, text=nd["label"], showarrow=False, font=dict(size=13, color=nd["color"]),
                      bgcolor="rgba(255,255,255,0.92)", bordercolor=nd["color"], borderwidth=bw, borderpad=3, captureevents=True))
fig.update_layout(annotations=_anns, paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF",
                  margin=dict(l=6, r=6, t=8, b=6), height=560, dragmode="pan")
...
C.note("<b>The connectome (constant in every view):</b> an edge = a <b>distinctive</b> association (PPMI, "
       ...
       "Because it is PPMI, not raw co-occurrence, it does <i>not</i> 'relate everything' — the hub ربب loses its spurious links. "
       "<b>Layouts</b> only move the nodes: <i>Meaning</i> = co-occurrence proximity (divine substrate); "
       "<i>Form</i> = spelling ring [HUMAN CONSTRUCT]; <i>Rarity</i> = frequency. Node placement r(form,meaning) = %.2f — "
       "near 0, so spelling tells you nothing about meaning. <i>Drag any label; scroll to zoom.</i>" % _R)
